...
確定方針（2026-07-23）:
  ...
  - 一意化: **(boarding_stop_id, store_id) ごとに最短(ride+walk2)の1行**に畳む
    （同一店×乗車停に複数経路があれば最短だけ残し、その transfer を保持。同点は直行優先）。
...
探索の起点は store_stops（店の近傍停＝降車停候補）。route_segments は隣接停ペアなので、
「直行=1区間」「hub経由=2区間(中間is_hub)」の**1〜2ホップ**を辿る。
...
def build_reach(store_stops: dict, seg_by_alight: dict, hubs: set, route_label: dict) -> tuple[list[dict], dict]:
    ...
    seg_by_alight: {alight_stop_id: [(boarding_stop_id, ride_min, route_id), ...]}  隣接1区間
    hubs:          {stop_id, ...}  is_hub=true の停
    route_label:   {route_id: label}
    """
    best: dict[tuple[int, int], dict] = {}  # (boarding, store) -> 最短行
    tally = {k: 0 for k in ("stores", "direct_cand", "hub_cand", "rows", "direct_rows", "hub_rows")}
    tally["stores"] = len(store_stops)

    def consider(boarding, store, alight, ride, walk2, transfer, via_hub, route_id):
        if boarding == alight:      # 乗車停＝降車停は無意味（乗らない）
            return
        total = ride + walk2
        key = (boarding, store)
        cur = best.get(key)
        # 最短(ride+walk2)。同点は直行(none)を優先。
        if (cur is None or total < cur["_total"]
                or (total == cur["_total"] and transfer == "none" and cur["transfer"] == "hub1")):
            best[key] = {
    ...
    for store, alights in store_stops.items():
        for alight, walk2 in alights:
            ...
            # (1) 直行: B →[1区間]→ alight
            for b, ride1, rt1 in ups:
                tally["direct_cand"] += 1
                consider(b, store, alight, ride1, walk2, "none", None, rt1)
            # (2) hub経由: B →[1区間]→ H(is_hub) →[1区間]→ alight
            for h, ride2, _rt2 in ups:
                if h not in hubs:
                    continue
                for b, ride1, rt1 in seg_by_alight.get(h, ()):
                    if b == h or b == alight:
                        continue
                    tally["hub_cand"] += 1
                    consider(b, store, alight, ride1 + ride2, walk2, "hub1", h, rt1)  # route_label=1本目(B→H)
...
_INSERT_SQL = (
    "INSERT INTO reach "
    "(boarding_stop_id, store_id, ride_min, walk2_min, transfer, via_hub_id, alight_stop_id, route_label) "
    "VALUES (:boarding_stop_id, :store_id, :ride_min, :walk2_min, :transfer, :via_hub_id, :alight_stop_id, :route_label)"
)
...
def main(dry_run: bool = False) -> None:
    ...
    with begin() as conn:  # 接続失敗時はパスワードを伏せて再送出（batch/db.py）
        ...
        for r in conn.execute(text("SELECT route_id, boarding_stop_id, alight_stop_id, ride_min FROM route_segments")):
            seg_by_alight[r.alight_stop_id].append((r.boarding_stop_id, r.ride_min, r.route_id))
